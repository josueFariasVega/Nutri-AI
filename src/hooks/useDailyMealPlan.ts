...
export function useDailyMealPlan() {
  ...
  const saveHistoricalData = useCallback((planData: DailyPlanData): void => {
    try {
      ...
      logger.log(`📊 Datos históricos guardados para ${planData.date}:`, historicalRecord);
    } catch (error) {
  ...
  const convertNutritionPlanToMeals = useCallback((plan: NutritionPlan, completedMeals: string[] = []): DailyMeal[] => {
    ...
    const addRealVarietyToFoods = (foods: any[], mealType: string) => {
      ...
      const foodVariationsByMeal = {
        breakfast: {
          0: ['Tortilla española con espinacas', 'Tostada integral con aguacate', 'Yogurt griego con granola'],
          1: ['Avena con frutas del bosque', 'Huevo revuelto con champiñones', 'Smoothie verde detox'],
          2: ['Pancakes de avena', 'Tostada con queso fresco', 'Batido de proteínas'],
          3: ['Cereal integral con leche', 'Croissant integral', 'Café con leche'],
          4: ['Bowl de açaí', 'Pan de centeno tostado', 'Infusión de hierbas'],
          5: ['Muesli casero', 'Bagel integral', 'Té verde matcha'],
          6: ['French toast saludable', 'Yogurt con miel', 'Zumo natural naranja']
        },
        lunch: {
          0: ['Ensalada César con pollo', 'Arroz integral con verduras', 'Sopa de lentejas'],
          1: ['Pasta integral con pesto', 'Salmón a la plancha', 'Gazpacho andaluz'],
          2: ['Quinoa con vegetales', 'Pechuga de pavo', 'Crema de calabaza'],
          3: ['Risotto de champiñones', 'Merluza al horno', 'Ensalada mixta'],
          4: ['Tacos de pescado', 'Pollo al curry', 'Sopa miso'],
          5: ['Bowl mediterráneo', 'Atún sellado', 'Vichyssoise'],
          6: ['Paella de verduras', 'Lubina a la sal', 'Consomé de pollo']
        },
        dinner: {
          0: ['Salmón con espárragos', 'Puré de coliflor', 'Infusión digestiva'],
          1: ['Pechuga pollo hierbas', 'Verduras al vapor', 'Té relajante'],
          2: ['Pescado blanco plancha', 'Ensalada verde', 'Manzanilla'],
          3: ['Tofu salteado', 'Brócoli al vapor', 'Té de jengibre'],
          4: ['Sepia a la plancha', 'Calabacín relleno', 'Tila natural'],
          5: ['Pollo al limón', 'Judías verdes', 'Valeriana'],
        ...
        snacks: {
          ...
          2: ['Pistachos sin sal', 'Plátano maduro', 'Hummus casero'],
          3: ['Anacardos tostados', 'Kiwi dorado', 'Requesón'],
          4: ['Mix frutos secos', 'Naranja navel', 'Cottage cheese'],
          5: ['Avellanas crudas', 'Uvas rojas', 'Yogurt griego'],
          6: ['Semillas calabaza', 'Melocotón', 'Ricotta light']
        }
      };
  ...
  const generateDailyPlan = useCallback(async (preserveCompletedMeals: boolean = true): Promise<void> => {
    ...
    try {
      ...
      if (!nutritionPlan) {
        setError('No se encontró un plan nutricional. Por favor, completa el cuestionario primero.');
        setLoading(false);
        return;
      }
      ...
      const dailyMeals = await convertNutritionPlanToMealsWithAPI(nutritionPlan, existingCompletedMeals)
        .catch(error => {
          logger.warn('API falló, usando método tradicional:', error);
...
saveHistoricalDataRef.current = saveHistoricalData;
  ...
  const setupPeriodicCheck = useCallback((): (() => void) => {
    const checkInterval = setInterval(async () => {
      ...
      if (currentPlan && currentPlan.date !== currentDate) {
        logger.log(`🔄 Cambio de día detectado: ${currentPlan.date} → ${currentDate}`);
  ...
  const setupServiceWorker = useCallback((): (() => void) => {
    if ('serviceWorker' in navigator) {
        ...
        const onMessage = (event: MessageEvent) => {
          ...
          switch (type) {
            ...
            case 'DAILY_RESET_ERROR':
              logger.error('❌ Service Worker reporta error en reset:', data);
              break;
              
            case 'STATUS_RESPONSE':
              logger.log('📊 Service Worker status:', data);
              break;
              
            default:
              logger.log('📨 Service Worker mensaje:', type, data);
          }
        };
      ...
      logger.warn('⚠️ Service Workers no soportados en este navegador');
    }
  ...
  const setupFallbackReset = useCallback((): (() => void) => {
    ...
    logger.log(`⏰ Fallback: Reset programado en ${Math.round(timeUntilUpdate / 1000 / 60)} minutos`);

    const timeoutId = setTimeout(async () => {
      logger.log('🌅 Ejecutando reset automático (fallback)');
...
useEffect(() => {
  ...
  const initializePlan = async () => {
    ...
    // ✅ Listener para eventos del Service Worker (evita problemas de hooks)
...
}, [setupServiceWorker, setupFallbackReset, handleServiceWorkerReset, handleVisibilityChange, handleFocus]); // ✅ Agregar las dependencias

/**
 * Verificación periódica del cambio de día
 */
useEffect(() => {
  const checkInterval = setInterval(async () => {
    const currentDate = getCurrentDate();
...
/**
 * Fuerza la verificación y reset del plan diario
 */
...
/**
 * Progreso de calorías como porcentaje
 */
...
/**
 * Fuerza la regeneración del plan
 */
...
/**
 * Actualiza el nivel de hidratación
 */
...
const toggleMealCompletion = useCallback((mealId: string, itemId: string): void => {
  ...
  // ✅ NOTIFICACIÓN DE COMIDA COMPLETADA
...
/**
 * Fuerza la regeneración del plan nutricional desde la API
 */
const regenerateNutritionPlan = useCallback(async (): Promise<void> => {
  try {
    logger.log('🔄 Forzando regeneración del plan nutricional desde API...');
    ...
    logger.error('❌ Error forzando regeneración del plan nutricional:', error);
    throw error;
  }
}, []);
...
return meals.map(meal => {
  ...
  switch (meal.id) {
    ...
    default:
      ...
      else if (meal.meal?.includes('Mañana') || meal.meal?.includes('Merienda')) restoredMeal.icon = Apple;
